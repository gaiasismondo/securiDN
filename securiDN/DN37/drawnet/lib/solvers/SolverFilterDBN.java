...
/**
 * SolverFilterDBN is a solver filter that converts a given model into an Octave script.
 */
public class SolverFilterDBN extends SolverFilter
{

	private final String DESCR = "SecuriDN ---> Octave";
	private final String OUTPUT_FILE = "OctaveScript_";

	/**
	 * Constructor.
	 */
	public SolverFilterDBN(){
		super();
	 ...
	 */
	public SolverFilterDBN( SolverListener listener ){
		super( listener );
		setDescription( DESCR );
	}
	...
	private void print(String messaggio){
	 ...
	 /**
	  * Visits the the model and prints properties.
	  */
	private void mainVisit(){
		Enumeration<ElementInstance> enumeration = model_.getMainElement().subElementsEnum();
		...
		while (enumeration.hasMoreElements()) {
			...
			if(elementType.getId().equals("Arc")){
				...
				if(fromRef == null || toRef == null)
					continue;
				ElementInstance fromInstance = model_.getMainElement().getSubElementByPath(fromRef.toString());
				ElementInstance toInstance = model_.getMainElement().getSubElementByPath(toRef.toString());
