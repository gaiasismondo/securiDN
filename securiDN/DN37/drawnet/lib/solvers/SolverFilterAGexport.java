...
public class SolverFilterAGexport extends SolverFilter 
{

  private final String DESCR = "AG export";
   ...
   */
  public SolverFilterAGexport( SolverListener listener ) 
  {
    	super( listener );
    	setDescription( DESCR );
  }
  ...
  private void selectAsset()
  {
	...
	ElementInstance elementInstance;
  	ElementType elementType;
	int i, scelta, scelta2, returnVal;
	File chosenFile;
	boolean trovato = false;	

	String[] scelte = {"choose AG file", "remove AG file", "export all", "cancel"};
	...
	do
	{
		scelta = JOptionPane.showOptionDialog(null, assetPanel, "Select an asset and choose the AG file to export.", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, scelte, "choose AG file");

		// INSERIMENTO NOMI DEI FILE
		if (scelta == 0 && assetTable.getSelectedRow() >= 0)
		{
			returnVal = jfc.showSaveDialog(DrawNET.getApplication().getMainFrame());
			if (returnVal == JFileChooser.APPROVE_OPTION) 
			{
				chosenFile = jfc.getSelectedFile();

				if (chosenFile.exists())
					scelta2 = JOptionPane.showConfirmDialog(null, "overwrite " + chosenFile.getName() +"?", chosenFile.getName() + " already exists!", JOptionPane.YES_NO_OPTION);
				else
					scelta2 = JOptionPane.YES_OPTION;

				if (scelta2 == JOptionPane.YES_OPTION)
				{
					i = fileList.indexOf(chosenFile);
					if (i == -1)
					{
						assetTable.setValueAt(chosenFile.getName(), assetTable.getSelectedRow(), 1);
						fileList.set(assetTable.getSelectedRow(), chosenFile);
					}
					else
						JOptionPane.showMessageDialog(null, chosenFile.getName() + " already associated with " + assetList.get(i), "error", JOptionPane.ERROR_MESSAGE, null);
				}
			}
		}

		// RIMUOVE NOMI DEI FILE
		if (scelta == 1 && assetTable.getSelectedRow() >= 0)
		{
			assetTable.setValueAt("", assetTable.getSelectedRow(), 1);
			fileList.set(assetTable.getSelectedRow(), null);
		}

		if ( (scelta == 0 || scelta ==1) && assetTable.getSelectedRow() == -1)
			JOptionPane.showMessageDialog(null, "No asset selected.", "error", JOptionPane.ERROR_MESSAGE, null);
	}
	...
	if (scelta == 2)
	{
		for (i=0; i < assetList.size(); i++)
		{
			...
			if (fileList.get(i) != null)
			{
				...
				this.AGexport(chosenAsset, chosenFile);
				trovato = true;
				JOptionPane.showMessageDialog(null, chosenAsset + " exported in " + chosenFile.getName(), "success", JOptionPane.INFORMATION_MESSAGE, null);
			}
		}

		if (! trovato)
			JOptionPane.showMessageDialog(null, "No AG exported.", "error", JOptionPane.ERROR_MESSAGE, null);
	}
  }
  
  private void AGexport(String assetName, File assetFile)
  {
	Enumeration<ElementInstance> enumeration;
	ElementInstance elementInstance, asset, main;
	ElementType elementType, mainType, layerType;
	XMLFormalismFilter xmlFormalismFilter;
	Formalism formalism;

	asset = arch.getSubElement(assetName);

	// CREATE EMPTY AG
	xmlFormalismFilter = new XMLFormalismFilter("xml/formalism/SecuriDN/AG/AG.fdl");
      	formalism = xmlFormalismFilter.read();
      	model = new Model(formalism);
      	mainType = formalism.getMainElement();
      	main = new ElementInstance("RootAG", mainType, true);
      	model.setMainElement(main);

	layerType = formalism.getSubElement("G_Layer", true);
	elementInstance = new ElementInstance("G_Layer1", layerType, true);
	elementInstance.setPropertyValue("Label", new StringPropertyValue("Layer 1"));
	model.getMainElement().addSubElement(elementInstance);
  ...
  {
	// COPIA I NODI DELL'ASSET NELL'AG
	...
	layer = new ElementRefPropertyValue(model.getSubElement("G_Layer1"));
	...
	enumeration = asset.subElementsEnum();
		...
		if (elementType.getParent("_root.AG.AGNode") != null)
		{
			...
			node = new ElementInstance(elementInstance.getId(), model.getMainElement().getElementType().getSubElement(elementType.getId()), true);
			...
			if (fromConnections != null)
			{
				...
				for (i=0; i<fromConnections.size(); i++)
				{
					...
					fap = model.getMainElement().getElementInstanceAbsolutePath() + "." + fap;
					//System.out.println("fap: " + fap);
			...
			if (! model.getMainElement().addSubElement(node))
  ...
  private void AGarcBuild(ElementInstance asset)
  {
	...
	layer = new ElementRefPropertyValue(model.getMainElement().getSubElement("G_Layer1"));
	...
	enumeration = asset.subElementsEnum();
		...
		if (elementType.getParent("_root.AG.AGArc") != null)
		{
			...
			arc = new ElementInstance(elementInstance.getId(), model.getMainElement().getElementType().getSubElement("Arc"), true);
			...
			from = from.replace(asset.getElementInstanceAbsolutePath(), model.getMainElement().getElementInstanceAbsolutePath());
			//System.out.println("from: " + from);
			...
			to = to.replace(asset.getElementInstanceAbsolutePath(), model.getMainElement().getElementInstanceAbsolutePath());
			//System.out.println("to: " + to);
			...
			if (! model.getMainElement().addSubElement(arc))
  ...
  private void save(File file)
  {
	...
	String FileName, PureName, PurePath, CommonPrefix;
	MXFile mxf;
	XMLModelFilter xmmf;
	File contentDir;
	...
	CommonPrefix = (DrawNETDeepConfig.ProjectsFileHolderDir.equals("") 
		? "" 
		: DrawNETDeepConfig.ProjectsFileHolderDir + "/" ) + PureName;
	print("CommonPrefix: " + CommonPrefix);

        contentDir = new File(PurePath + CommonPrefix);
	contentDir.mkdirs();
	...
	mxf.setFDLname("SecuriDN/AG/AG.dfs");
